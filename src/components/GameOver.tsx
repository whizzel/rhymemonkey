  ...
  return (
    <>
      <style>{`
        ...
        .go-body{padding:26px 22px 22px;display:flex;flex-direction:column;align-items:center;gap:18px;position:relative;z-index:1;}
        .go-title{font-family:'Orbitron',monospace;font-weight:900;font-size:40px;background:linear-gradient(135deg,#ff2a2a,#c8e0ff 60%,#00d4ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:titleGlow 2s ease-in-out infinite alternate;letter-spacing:.06em;}
        @keyframes titleGlow{from{filter:drop-shadow(0 0 8px rgba(255,30,30,0.6))}to{filter:drop-shadow(0 0 8px rgba(0,180,255,0.6))}}
        .go-perf{font-family:'Orbitron',monospace;font-size:16px;color:#6a9ac0;letter-spacing:.1em;}
        .go-stars{display:flex;gap:8px;}
        .go-star{font-size:28px;filter:grayscale(1) brightness(.3);transition:filter .3s,transform .3s;}
        ...
        .go-stats{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;width:100%;}
        .go-sc{background:#060f1e;border:1px solid #1a3a6a;border-radius:8px;padding:11px 14px;display:flex;flex-direction:column;gap:2px;}
        .go-sl{font-size:8px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;color:#3a6a9a;}
        .go-sv{font-family:'Orbitron',monospace;font-size:26px;color:#c8e0ff;line-height:1;}
        .go-ps{width:100%;background:#060f1e;border:1px solid #1a3a6a;border-radius:8px;padding:11px 14px;}
        .go-pl{font-size:8px;font-weight:800;letter-spacing:.2em;text-transform:uppercase;color:#3a6a9a;margin-bottom:8px;}
        .go-pr{display:flex;justify-content:space-around;}
        .go-pc{text-align:center;}
        .go-pc-l{font-size:9px;font-weight:800;color:#3a6a9a;margin-bottom:2px;}
        .go-pc-v{font-family:'Orbitron',monospace;font-size:18px;color:#c8e0ff;}
        ...
        .go-grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px;width:100%;}
      `}</style>
      <div className={`go-wrap ${vis ? 'vis' : ''}`}>
        <div className="go-body">
          <div className="go-title">GAME OVER</div>
          <MonkeyCharacter mood={perf.mood} size={100} />
          ...
          <div className="go-div" />
          <button type="button" className="g-btn2 b-red" style={{ fontSize: 16, padding: '15px' }} onClick={onPlayAgain}>⚡ PLAY AGAIN</button>
          <div className="go-grid2">
            <button type="button" className="g-btn2 b-dark" onClick={onBackToMenu}>⌂ MENU</button>
            <button type="button" className="g-btn2 b-blue" onClick={onShowLeaderboard}>🏆 RANKINGS</button>
          </div>
        </div>
      </div>
    </>
